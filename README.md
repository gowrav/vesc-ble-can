...
## Firmware & UI Integrations

This repository includes **reference integrations** for using `vesc-ble-can`
across firmware and UI layers.

These are **not installed via pip** and are intended as examples.

- **VESC Lisp (Firmware)**  
  Handles `COMM_CUSTOM_APP_DATA` on CAN-connected VESC nodes.

- **QML (VESC Tool UI)**  
  Simple control panel for sending custom commands via BLE.

See [`integrations/README.md`](integrations/README.md) for details, protocol
mapping, and usage instructions.
